...
st.title("📽️ Pharma Safety Twin Playback – v12 with Uploaded Emergency Siren")
...
# Encode uploaded siren for autoplay
def play_local_siren():
    file_path = "loud-emergency-alarm-54635.mp3"
...
if play:
    for ts in time_steps:
        ...
        reactor_status = snapshot[snapshot["Equipment"] == "Reactor"]["Status"].values[0]
        if reactor_status in ["Warning", "High Risk"] and not siren_started:
            audio_area.markdown(play_local_siren(), unsafe_allow_html=True)
            siren_started = True
