...
# Use HTML audio (for better compatibility on Streamlit Cloud)
audio_tag = '''
<audio autoplay>
...
if play:
    for ts in time_steps:
        ...
        status_table.subheader(f"📋 Equipment Status at {ts}")
        status_table.dataframe(snapshot)

        # Sound alert for "Warning"
        if "Warning" in snapshot["Status"].values:
            alert_audio.markdown(audio_tag, unsafe_allow_html=True)
        else:
            alert_audio.empty()
